...
subtitle: "Estudio de caso 1 — Curso de Estadística Bayesiana, Universidad Nacional de Colombia"
author:
- Lubin Abaunza 
- Email   labaunza@unal.edu.co
date: ''
output:
  pdf_document:
    highlight: default
...
**Nota**: una observación de una variable de interés se denomina *outier* extremo si la observación es bien sea inferior a $q_1 - 3.0 RI$ o superior a $q_3 + 3.0 RI$, donde $q_1$ y $q_3$ son el percentil 25 y 75 de la variable, respectivamente, y $RI = q_3 - q_1$ es el rango intercuartílico. 

```{r,eco=F}
# Conteo de victimas
# Base de datos
dat_bogota <- read.csv("C:/Users/labau/OneDrive/Escritorio/UN/Estadistica Bayesiana/Caso de estudio # 1/victimas.csv")
...
\begin{align*}
    y_{k,i}\mid\theta_k &\overset{\text{iid}}{\sim} \textsf{Poisson}(\theta_k), \quad i = 1,\ldots,n_k, \\
...
$$
\theta_1^{(1)},\ldots,\theta_1^{(B)} \thicksim \textsf{Gamma}(208.01,115.01)
\qquad\text{y}\qquad
\theta_2^{(1)},\ldots,\theta_2^{(B)} \thicksim \textsf{Gamma}(539.01,237.01)\,,
$$
y se calcula 
$$
...
**Se estima que la tasa de victimización de las mujeres es, en promedio, 26 puntos superior a la de los hombres, tomando como referencia la tasa de los hombres. Además, con una probabilidad del 95%, esta diferencia relativa se encuentra entre 7 y 47 puntos. Estos resultados evidencian que, bajo el modelo propuesto, la tasa promedio de víctimas en mujeres fue significativamente mayor que la observada en hombres en Bogotá D. C. durante 2022.**
...
***Las inferencias sobre la tasa de cambio que compara la victimización entre mujeres y hombres, tomando como referencia a los hombres, resultan robustas frente a la información externa incorporada al modelo. Esto se debe a que la distribución posterior, así como sus correspondientes medidas resumen, no presenta variaciones sustanciales entre las distintas distribuciones previas consideradas.***
...
$$
\tilde{y}_{k,1}^{(b)},\ldots,\tilde{y}_{k,n_k}^{(b)}\mid\theta_k^{(b)} \thicksim \textsf{Poisson}(\theta_k^{(b)})\,,\qquad b=1,\ldots,B\,.
$$
...
***El modelo propuesto permite caracterizar adecuadamente la media del conteo de víctimas tanto en hombres como en mujeres ($ppp = 0.477$ y $ppp = 0.482$, respectivamente). Por otra parte, aunque el modelo describe de manera satisfactoria la variabilidad del conteo de víctimas en hombres ($ppp = 0.081$), existe evidencia importante de que subestima dicha variabilidad en el caso de las mujeres ($ppp = 0.000$).***
...
\begin{align*}
    y_{k,i}\mid\theta_k &\thicksim \textsf{Poisson} (\theta_k)\,,\quad i = 1,\ldots,n_k\,,\qquad k = 1,2\,.\\
\end{align*}
...
$$
\tilde{y}_{k,1},\ldots,\tilde{y}_{k,n_k}\thicksim \textsf{Poisson}(\hat{\theta}_{\text{mle}\,k})\,,\qquad k=1,2\,.
$$
...
***Ambos enfoques producen inferencias prácticamente idénticas sobre la tasa de cambio que compara la victimización entre mujeres y hombres, tomando como referencia a los hombres. Este resultado no es inesperado, dado que la distribución previa empleada en el análisis Bayesiano es difusa y, además, los tamaños de muestra para hombres y mujeres son suficientemente grandes como para dominar las inferencias en un espacio paramétrico de baja dimensión.***
...
***Nuevamente, ambos enfoques producen tasas de cobertura para $\eta$ prácticamente idénticas y cercanas al 95%, tal como se esperaba. Estos resultados son alentadores, ya que en ambos casos la cobertura empírica coincide de manera adecuada con la cobertura nominal.***
...
***Al igual que en los análisis anteriores, las inferencias obtenidas bajo ambos paradigmas son muy similares. En la mayoría de los años, la estimación puntual de la tasa de cambio que compara la victimización entre mujeres y hombres, tomando como referencia a los hombres, es superior a cero —con excepción de 2013—, lo que sugiere una mayor victimización en mujeres durante el periodo 2012–2022. Sin embargo, dichas diferencias solo resultan estadísticamente significativas al 95% en 2016, 2018, 2019 y 2022. Las mayores diferencias se observan en 2016, mientras que las menores se presentan en 2013. Finalmente, la amplitud de los intervalos evidencia una mayor incertidumbre en la estimación de la tasa de cambio entre 2012 y 2016, la cual disminuye de manera considerable entre 2017 y 2022.***
